...
# 인증서를 바꾸지 않는다.
#
# 자동 업데이트는 "새로 받은 exe 가 지금 도는 exe 와 같은 인증서로 서명되었는가"를 보고 판단한다.
# 다른 키로 서명하면 이미 깔린 모든 복사본이 그 업데이트를 거부한다.
# 사용자가 갇히지는 않는다(받는 곳을 브라우저로 열어 준다) 하지만 한 번은 손으로 받아야 한다.
# 키를 잃어버렸거나 만료가 다가와 어쩔 수 없다면, 릴리즈 노트에 그 사실을 먼저 적을 것.
#
# 모든 릴리즈 exe 는 반드시 서명한다. 서명 없이 올라간 버전을 받은 사람은
# 대조할 기준이 없어져 그다음 업데이트부터 자동으로 받지 못한다.
